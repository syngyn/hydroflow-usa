    ...
    <div className="max-w-4xl mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 mt-1">Open issues from GitHub &mdash; <span className="font-mono text-sm">base44/syngynbase44</span></p>
        </div>
      ...
      {!loading && issues.length > 0 && (
        <div className="space-y-3">
          ...
          <p className="text-sm text-gray-400 text-right pt-2">{issues.length} open issue{issues.length !== 1 ? "s" : ""}</p>
        </div>
      )}
    </div>
  );
